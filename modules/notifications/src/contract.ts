 ...
 * and that is a door the boundary did not have before. This file is the door:
 * `@template/notifications` resolves to `createApp` and nothing else,
 * `@template/notifications/contract` resolves to here, and `dist/repository.js` resolves to nothing
 * at all.
 *
 * It re-exports types and only types. Nothing here may pull in the repository or the routing to
 * Email — the `exports` key would then be a formality while the whole module leaked through it.
 *
