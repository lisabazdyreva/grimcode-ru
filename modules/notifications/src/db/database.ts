 ...
 */
export function createDatabase(logger: Logger): (env: NotificationsEnv) => Promise<Pool> {
  const open = async (env: NotificationsEnv): Promise<Pool> => {
    ...
    const pool = new pg.Pool({
      ...
      application_name: 'notifications-service',
    });
 ...
 * cold start work.
 */
async function ensureDatabase(env: NotificationsEnv, logger: Logger): Promise<void> {
  const server = new pg.Pool({
    connectionString: env.maintenanceUrl,
    max: 1,
    idleTimeoutMillis: 5_000,
    connectionTimeoutMillis: 10_000,
    application_name: 'notifications-create',
  });
